...
import { PredictionEliminationGamesType, PredictionGroupGamesType, Matchup, Team, PointsSystemType } from "@/types/types";
...
export function handleRankingsChange(
  team: Team,
  setGroupGames: React.Dispatch<React.SetStateAction<PredictionGroupGamesType[]>>
) {
...
export function handleUnlockGroupPhase(
  ...
  setEliminationGames: (games: PredictionEliminationGamesType[]) => void,
...
export async function handleLockGroupPhase(
  tournamentId: number,
  groupGames: PredictionGroupGamesType[],
  eliminationGames: PredictionEliminationGamesType[],
  setGroupGamesLock: (lock: boolean) => void,
  setEliminationGames: (games: PredictionEliminationGamesType[]) => void,
  setChampion: (champ: Team | null) => void
  ...
  setGroupGames((prevGroups) =>
    prevGroups.map((group) => {
      ...
      const updatedGames = group.games.map((game) =>
        game.id === gameId ? { ...game, predicted_result: value } : game
      );
